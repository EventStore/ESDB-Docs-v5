...
- _Edit ACL_: Edit [the access control lists](security.mdx#access-control-lists) for a stream.
- _Add Event_: [Add a new event](/http-api/http-introduction#appending-events) to the stream.
- _Delete_: [Delete a stream](/http-api/http-introduction#deleting-a-stream) to the stream.
- _Query_:
...
The _Persistent Subscriptions_ tab shows an overview of [persistent subscriptions](/http-api/http-persistent) configured on streams. The button above the list do the following:
...
The _Users_ tab shows [the users defined in EventStoreDB](security.mdx#authentication), clicking an individual user shows a JSON representation of that users details.
